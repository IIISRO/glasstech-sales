...
function addService(event) {
  ...
  let lastService = document.getElementById(
    `package-${packageID}-service-${lastServiceID}`
  );
        ...
        <select required class="form-control" style="background: rgb(245, 245, 245); border: 0;" onchange="findUnit(event)" data-package_id="${packageID}" data-service_id="${newerServiceID}" name="package-${packageID}-service-${newerServiceID}-product" id="package-${packageID}-service-${newerServiceID}-product">
          ...
          ${(function listProducts() {
            ...
            for (let product of productsList) {
              options.push(
                `<option class="form-control" data-prodid="${product.id}" data-unit="${product.unit}" value="${product.id}">${product.name}</option>`
              );
            }
            return options;
          })()}
  ...
  if (lastServiceID == 1) {
    ...
    document
      .getElementById(`package-${packageID}-service-${lastServiceID}-index`)
      .insertAdjacentHTML("beforeBegin", removeBtn);
  }
...
function addPackage(event) {
  ...
  if (lastPackageID == 1) {
    ...
    `;
    document
      .getElementById(`package-${lastPackageID}-version`)
      .insertAdjacentHTML("beforeBegin", removeBtn);
    document.getElementById(
      `package-${lastPackageID}-version`
    ).innerText = `VER${lastPackageID}`;
  }
  if (newerPackageID > 1) {
    // versiya kodu  elave etmek
    document.getElementById(
      `package-${newerPackageID}-version`
    ).innerText = `VER${newerPackageID}`;
  }
...
function removeService(event) {
  ...
  let removedService = document.getElementById(
    `package-${removedServicePackageID}-service-${removedserviceID}`
  );
  CKEDITOR.instances[
    `package-${removedServicePackageID}-service-${removedserviceID}-detail`
  ].destroy();
  removedService.remove();

  let services = document
    .getElementById(`package-${removedServicePackageID}`)
    .getElementsByClassName(`services`);
...
function removePackage(event) {
  ...
  let removedPackageServices =
    removedPackage.getElementsByClassName(`services`);
  // butun ck lari sil
  for (let service of removedPackageServices) {
    CKEDITOR.instances[
      `package-${removedPackageID}-service-${service.dataset.service_id}-detail`
    ].destroy();
  }
...
function servicesReID(packageID, services) {
  ...
  for (let service of services) {
    CKEDITOR.instances[
      `package-${service.dataset.package_id}-service-${service.dataset.service_id}-detail`
    ].destroy();
  }
  ...
  for (let service of services) {
    ...
    let index = document.getElementById(
      `package-${service.dataset.package_id}-service-${service.dataset.service_id}-index`
    );
    index.innerText = newerServiceID;
    index.id = `package-${packageID}-service-${newerServiceID}-index`;
    // product
    let serviceProduct = document.getElementById(
      `package-${service.dataset.package_id}-service-${service.dataset.service_id}-product`
    );
    ...
    // detail
    let serviceDetail = document.getElementById(
      `package-${service.dataset.package_id}-service-${service.dataset.service_id}-detail`
    );
    ...
    // templates
    let descTemplates = document.getElementById(
      `package-${service.dataset.package_id}-service-${service.dataset.service_id}-templates`
    );
    ...
    // usermaterial
    let UsedServiceModalBTN = document.getElementById(
      `package-${service.dataset.package_id}-service-${service.dataset.service_id}-UsedMaterialBTN`
    );
    ...
    document.getElementById(
      `package-${service.dataset.package_id}-service-${service.dataset.service_id}-UsedMaterialList`
    ).id = `package-${packageID}-service-${newerServiceID}-UsedMaterialList`;

    // unit
    let serviceUnit = document.getElementById(
      `package-${service.dataset.package_id}-service-${service.dataset.service_id}-unit`
    );
    serviceUnit.id = `package-${packageID}-service-${newerServiceID}-unit`;
    // quantity
    let serviceQuantity = document.getElementById(
      `package-${service.dataset.package_id}-service-${service.dataset.service_id}-quantity`
    );
    ...
    // price
    let servicePrice = document.getElementById(
      `package-${service.dataset.package_id}-service-${service.dataset.service_id}-price`
    );
    ...
    // totalprice
    let serviceTotalPrice = document.getElementById(
      `package-${service.dataset.package_id}-service-${service.dataset.service_id}-totalprice`
    );
    serviceTotalPrice.id = `package-${packageID}-service-${newerServiceID}-totalprice`;

    // add button
    if (!--iterations) {
      let serviceAddButton = document.getElementById(
        `package-${service.dataset.package_id}-service-${service.dataset.service_id}-addbutton`
      );
      if (
        document.getElementById(
          `package-${service.dataset.package_id}-service-${service.dataset.service_id}-addbutton`
    ...
    // remove button
    let serviceRemoveButton = document.getElementById(
      `package-${service.dataset.package_id}-service-${service.dataset.service_id}-removebtn`
    );
...
function packagesReID() {
  ...
  for (let package of packages) {
    ...
    let version = document.getElementById(
      `package-${package.dataset.package_id}-version`
    );
    ...
    let totalPrice = document.getElementById(
      `package-${package.dataset.package_id}-totalprice`
    );
    totalPrice.id = `package-${newerPackageID}-totalprice`;

    // discount tax delv buttons
    let discountBtn = document.getElementById(
      `package-${package.dataset.package_id}-discountbtn`
    );
    let taxBtn = document.getElementById(
      `package-${package.dataset.package_id}-taxbtn`
    );
    let delvBtn = document.getElementById(
      `package-${package.dataset.package_id}-delvbtn`
    );
    ...
    // discount line
    let discountBody = document.getElementById(
      `package-${package.dataset.package_id}-discount`
    );
    if (discountBody != null) {
      document.getElementById(
        `package-${package.dataset.package_id}-discountfield`
      ).dataset.package_id = newerPackageID;
      document.getElementById(
        `package-${package.dataset.package_id}-discountfield`
      ).id = `package-${newerPackageID}-discountfield`;
      document.getElementById(
        `package-${package.dataset.package_id}-totalpricewdiscount`
      ).id = `package-${newerPackageID}-totalpricewdiscount`;
      discountBody.id = `package-${newerPackageID}-discount`;
    }
    // tax line
    let taxBody = document.getElementById(
      `package-${package.dataset.package_id}-tax`
    );
    if (taxBody != null) {
      document.getElementById(
        `package-${package.dataset.package_id}-taxprice`
      ).id = `package-${newerPackageID}-taxprice`;
      document.getElementById(
        `package-${package.dataset.package_id}-totalpricewtax`
      ).id = `package-${newerPackageID}-totalpricewtax`;
      taxBody.id = `package-${newerPackageID}-tax`;
    }
    // delv line
    let delvBody = document.getElementById(
      `package-${package.dataset.package_id}-delv`
    );
    if (delvBody != null) {
      document.getElementById(
        `package-${package.dataset.package_id}-delvfield`
      ).dataset.package_id = newerPackageID;
      document.getElementById(
        `package-${package.dataset.package_id}-delvfield`
      ).id = `package-${newerPackageID}-delvfield`;
      delvBody.id = `package-${newerPackageID}-delv`;
    }

    // addbutton
    if (!--iterations) {
      let packageAddBtn = document.getElementById(
        `package-${package.dataset.package_id}-addbtn`
      );
    ...
    // remove button
    let packageRemoveButton = document.getElementById(
      `package-${package.dataset.package_id}-removebtn`
    );
  ...
  if (packages.length == 1) {
    ...
    if (
      ...
      document
        .getElementById(`package-${package.dataset.package_id}-removebtn`)
        .remove();
    }
  }
}
...
function findUnit(event) {
  ...
  fetch(`/products/get-templates/${id}`, {
    ...
    .then((data) => {
      var templateSelect = $(
        `#package-${packageID}-service-${serviceID}-templates`
      );
...
function checkFloatInput() {
  $(".floatfield").on("input", function (event) {
    ...
    if (
      ...
      event.target.value.substring(1, 2) != "." &&
...
function calcPackageTotalPrice(calcPackageID) {
  let packageServicesPrices = document
    .getElementById(`package-${calcPackageID}`)
    .getElementsByClassName("servicetotalprice");
  ...
  if (packageTotalPrice > 0) {
    document.getElementById(`package-${calcPackageID}-totalprice`).innerText =
      packageTotalPrice;
  } else {
    document.getElementById(
      `package-${calcPackageID}-totalprice`
    ).innerText = 0;
  }
  if (
    document.getElementById(`package-${calcPackageID}-delvfield`) &&
    parseInt(
      document.getElementById(`package-${calcPackageID}-delvfield`).value
    )
  ) {
    document.getElementById(`package-${calcPackageID}-totalprice`).innerText =
      parseInt(
...
function calcDiscountTotalPrice(event, packageID) {
  ...
  let discountPrice = parseInt(
    document.getElementById(`package-${calcPackageID}-discountfield`).value
  );
  let packageTotalPrice = parseInt(
    document.getElementById(`package-${calcPackageID}-totalprice`).innerText
  );
  if (packageTotalPrice - discountPrice >= 1) {
    document.getElementById(
      `package-${calcPackageID}-totalpricewdiscount`
    ).innerText = packageTotalPrice - discountPrice;
  } else {
    document.getElementById(
      `package-${calcPackageID}-totalpricewdiscount`
    ).innerText = packageTotalPrice;
  }
  if (document.getElementById(`package-${calcPackageID}-tax`) != null) {
    calcTaxTotalPrice(calcPackageID);
  }
...
function calcServiceTotalPrice(event) {
  ...
  let servicePrice = parseFloat(
    document.getElementById(
      `package-${calcPackageID}-service-${calcServiceID}-price`
    ).value
  );
  let serviceQuantity = parseFloat(
    document.getElementById(
      `package-${calcPackageID}-service-${calcServiceID}-quantity`
    ).value
  );

  if (servicePrice * serviceQuantity >= 1) {
    document.getElementById(
      `package-${calcPackageID}-service-${calcServiceID}-totalprice`
    ).innerText = parseInt(servicePrice * serviceQuantity);
  }
  calcPackageTotalPrice(calcPackageID);
}

function calcTaxTotalPrice(packageID) {
  let packageTotalPriceWDiscount = document.getElementById(
    `package-${packageID}-totalpricewdiscount`
  );
  if (packageTotalPriceWDiscount != null) {
    let packageTaxPrice = document.getElementById(
      `package-${packageID}-taxprice`
    );
    let packageTotalPriceWTAX = document.getElementById(
      `package-${packageID}-totalpricewtax`
    );
    if (parseInt(packageTotalPriceWDiscount.innerText) >= 1) {
      let taxPrice = parseInt(
        (parseInt(packageTotalPriceWDiscount.innerText) * 18) / 100
      );
      packageTaxPrice.innerText = taxPrice;
      packageTotalPriceWTAX.innerText =
        parseInt(packageTotalPriceWDiscount.innerText) + taxPrice;
    } else {
      packageTaxPrice.innerText = 0;
      packageTotalPriceWTAX.innerText = 0;
    }
  } else {
    let packageTotalPrice = document.getElementById(
      `package-${packageID}-totalprice`
    );
    let packageTaxPrice = document.getElementById(
      `package-${packageID}-taxprice`
    );
    ...
    if (parseInt(packageTotalPrice.innerText) >= 1) {
      let taxPrice = parseInt(
        (parseInt(packageTotalPrice.innerText) * 18) / 100
      );
      packageTaxPrice.innerText = taxPrice;
      packageTotalPriceWTAX.innerText =
        parseInt(packageTotalPrice.innerText) + taxPrice;
    } else {
      packageTaxPrice.innerText = 0;
      packageTotalPriceWTAX.innerText = 0;
    }
...
function packageDiscountField(event) {
  ...
  if (event.target.classList.contains("btn-secondary")) {
    ...
    <tr style="height: 26.05pt;">
      <td width="585" colspan="5" style="width: 439.1pt; border-right: 1pt solid windowtext; border-bottom: 1pt solid windowtext; border-left: 1pt solid windowtext; border-image: initial; border-top: none; background: rgb(229, 229, 229); padding: 0cm 5.4pt; height: 26.05pt;">
        ...
        <span style="display: flex; align-items: center; justify-content:center; ">
         <input required onchange="calcDiscountTotalPrice(event,null)" data-package_id = ${
           event.target.dataset.package_id
         } class='form-control integerfield' placeholder='0' id="package-${
      event.target.dataset.package_id
    }-discountfield" style="padding:0px; width:50px; background: rgb(229, 229, 229); border:0px; margin: 0cm; font-size: 12pt; text-align: center; font-weight: bold;"><b>AZN</b>
        </span>  
        </td>
    </tr>
    <tr style="height: 26.05pt;">
      ...
      <td width="86" colspan="2" style="width: 64.15pt; border-top: none; border-left: none; border-bottom: 1pt solid windowtext; border-right: 1pt solid windowtext; background: rgb(229, 229, 229); padding: 0cm 5.4pt; height: 26.05pt;">
        <p style="margin: 0cm; font-size: 12pt; text-align: center; font-weight: bold;"><span id="package-${
          event.target.dataset.package_id
        }-totalpricewdiscount">${
      document.getElementById(
        `package-${event.target.dataset.package_id}-totalprice`
      ).innerText
    }</span> AZN</p>
      </td>
    </tr>
    ...
    if (
      document.getElementById(
        `package-${event.target.dataset.package_id}-tax`
      ) == null
    ) {
      event.target.parentElement.parentElement.parentElement.insertAdjacentHTML(
        "afterEnd",
        discount
      );
    } else {
      document
        .getElementById(`package-${event.target.dataset.package_id}-tax`)
        .insertAdjacentHTML("beforeBegin", discount);
    }
    event.target.classList.remove("btn-secondary");
    event.target.classList += " btn-warning";
  } else {
    document
      .getElementById(`package-${event.target.dataset.package_id}-discount`)
      .remove();
...
function packageTaxField(event) {
  if (event.target.classList.contains("btn-secondary")) {
    ...
    if (
      document.getElementById(
        `package-${event.target.dataset.package_id}-discount`
      ) == null
    ) {
      event.target.parentElement.parentElement.parentElement.insertAdjacentHTML(
        "afterEnd",
      ...
      document
        .getElementById(`package-${event.target.dataset.package_id}-discount`)
        .insertAdjacentHTML("afterEnd", tax);
    }
    event.target.classList.remove("btn-secondary");
    event.target.classList += " btn-warning";
    calcTaxTotalPrice(event.target.dataset.package_id);
...
function packageDelvField(event) {
  ...
  if (event.target.classList.contains("btn-secondary")) {
    ...
    if (
      document.getElementById(
        `package-${event.target.dataset.package_id}-delv`
      ) == null
    ) {
      event.target.parentElement.parentElement.parentElement.insertAdjacentHTML(
        "beforeBegin",
      ...
      document
        .getElementById(`package-${event.target.dataset.package_id}-delv`)
        .insertAdjacentHTML("afterEnd", delv);
    }
    event.target.classList.remove("btn-secondary");
    event.target.classList += " btn-warning";
  } else {
...
function setTemplate(event) {
  if (event.target.value) {
    CKEDITOR.instances[
      `package-${event.target.dataset.package_id}-service-${event.target.dataset.service_id}-detail`
...
createOfferForm.addEventListener("submit", (e) => {
  ...
  for (let package of $(".packages")) {
    ...
    if (document.getElementById(`package-${packageID}-tax`) != null) {
      newerPackage["tax"] = parseInt(
        document.getElementById(`package-${packageID}-taxprice`).innerText
      );
    } else {
      newerPackage["tax"] = null;
    }
    if (document.getElementById(`package-${packageID}-discount`) != null) {
      newerPackage["discount"] = document.getElementById(
        `package-${packageID}-discountfield`
      ).value;
    } else {
      newerPackage["discount"] = null;
    }
    if (document.getElementById(`package-${packageID}-delv`) != null) {
      newerPackage["delv"] = document.getElementById(
        `package-${packageID}-delvfield`
      ).value;
    } else {
    ...
    for (let service of package.getElementsByClassName("services")) {
      ...
      newerService["product"] = parseInt(
        document.getElementById(
          `package-${packageID}-service-${serivceID}-product`
        ).value
      );
      newerService["detail"] =
        CKEDITOR.instances[
          `package-${packageID}-service-${serivceID}-detail`
        ].getData();
      newerService["quantity"] = parseFloat(
        document.getElementById(
          `package-${packageID}-service-${serivceID}-quantity`
        ).value
      );
      newerService["price"] = parseFloat(
        document.getElementById(
          `package-${packageID}-service-${serivceID}-price`
        ).value
      );
      let newUsedMaterials = [];
      let usedMaterialList = document
        .getElementById(
          `package-${packageID}-service-${serivceID}-UsedMaterialList`
        )
        .getElementsByClassName("usedmaterial");
      for (let used of usedMaterialList) {
        var usedMaterial = {
          product_id: parseInt(
            used.getElementsByClassName("usedprod")[0].value
          ),
          quantity: parseFloat(
            used.getElementsByClassName("usedquantity")[0].value
          ),
  ...
  var offerInps = $("#offer_pay_delv_cond");
  let offerinpsVal = $("#offer_pay_delv_cond").val();
  console.log(offerInps, offerinpsVal, "offer inputs of alizamin");
