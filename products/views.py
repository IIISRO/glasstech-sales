    ...
    def get(self, request):
        ...
        for product in products:
            ...
            if request.GET.get('date'):
                ...
                services = OfferRevisionPackageService.objects.filter(package__revision__is_active=True).filter(
                    package__revision__offer__status='Uğurlu').filter(product=product).filter(
                    created_at__gte=date(from_date[2], from_date[1], from_date[0]),
                    created_at__lte=date(to_date[2], to_date[1], to_date[0])).order_by('created_at')
            else:
                services = OfferRevisionPackageService.objects.filter(package__revision__is_active=True).filter(
                    package__revision__offer__status='Uğurlu').filter(product=product)

            for service in services:
                if service.services_used_products.exists():
                    for used_prod in service.services_used_products.all():
                        product_item = {'id': service.product.id, 'name': service.product.name,
                                        'unit': service.product.unit, 'category': service.product.category,
                                        'quantity': 0.0, 'total': 0,
                                        'useds': {'id': '', 'name': '', 'unit': '', 'category': '', 'quantity': 0,
                                                  'total': 0}}
                        ...
                        if len(temp) > 0:
                            added = False
                            for prod in temp:
                                if prod['name'] == product_item['name'] and prod['useds']['name'] == \
                                        product_item['useds']['name']:
                    ...
                    product_item = {'id': service.product.id, 'name': service.product.name,
                                    'unit': service.product.unit, 'category': service.product.category, 'quantity': 0.0,
                                    'total': 0,
                                    'useds': {'id': '', 'name': '', 'unit': '', 'category': '', 'quantity': 0,
                                              'total': 0}}
                    product_item['quantity'] = service.quantity
                    ...
                    if len(temp) > 0:
                        added = False
                        for prod in temp:
                            if prod['name'] == product_item['name'] and prod['useds']['name'] == product_item['useds'][
                                'name']:
                                prod['quantity'] += product_item['quantity']
                                prod['total'] += product_item['total']
                                added = True
            ...
            if services.exists():
                ...
                products_list.append(
                    {'id': product.id, 'name': product.name, 'unit': product.unit, 'category': product.category,
                     'quantity': 0, 'total': 0,
                     'useds': {'id': '', 'name': '', 'unit': '', 'category': '', 'quantity': 0, 'total': 0}})
